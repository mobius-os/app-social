...
test('inviting into an existing group uses its exact id and chosen deployment', async () => {
  ...
  globalThis.fetch = async (url, options) => {
    assert.equal(url, '/api/services/social/groups/group-id/members')
...
test('deleting uses the group authority, never a local storage wipe', async () => {
  ...
  globalThis.fetch = async (url, options) => {
    assert.equal(url, '/api/services/social/groups/group-id')
...
test('message-request decisions use server-owned authenticated actions', async () => {
  setToken('scoped-test')
  const expected = [
    ['/api/services/social/requests/dm/peer.example/accept', { status: 'accepted' }],
    ['/api/services/social/requests/dm/peer.example/decline', { status: 'declined' }],
    ['/api/services/social/requests/dm/peer.example/block', { status: 'blocked' }],
  ]
  globalThis.fetch = async (url, options) => {
...
test('group invitations are accepted or declined through group authority', async () => {
  setToken('scoped-test')
  const expected = [
    ['/api/services/social/groups/group-id/accept', { status: 'accepted' }],
    ['/api/services/social/groups/group-id/decline', { status: 'declined', host_notified: false }],
  ]
  globalThis.fetch = async (url, options) => {
    const [nextUrl, receipt] = expected.shift()
    assert.equal(url, nextUrl)
